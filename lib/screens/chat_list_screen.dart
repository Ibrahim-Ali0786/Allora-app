...
class _ChatListScreenState extends ConsumerState<ChatListScreen> {
  List<Room> _getFilteredRooms(Client client, NetworkHubState networkState) {
    // 1. Get a set of networks that are ACTUALLY connected right now
    final connectedNetworks = networkState.networks
        .where((n) => n.status == NetworkStatus.connected)
        .map((n) => n.meta.id)
        .toSet();

    // 2. Filter the Matrix rooms
    return client.rooms.where((room) {
      if (room.membership != Membership.join) return false;
      if (room.isSpace) return false;

      // Identify which network the room belongs to
      final networkId =
          BridgeRoomClassifier.getNetworkForRoom(room, client: client);

      // If it's a bridge room, ONLY show it if that network is currently connected
      if (networkId != null) {
        // Also hide the management bots from the general chat list
        if (room.displayname
            .toLowerCase()
      ...
      // Allow native Matrix chats (if any)
      return true;
    }).toList()
      ..sort((a, b) {
